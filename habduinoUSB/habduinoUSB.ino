...
char comment[3]={
  ' ', ' ', '\0'};
// for Pi comms
char PiString[ ] = "No data received";
...
void prepare_data() {
  ...
  if (Serial.available()) {
    ...
    PiString[17] = inData[17];
  }
  Serial.println("Pi Message:");
...
void tx_aprs()
{
  ...
  if(tempsensors==1)
  {
    ...
    "WIDE1", 1, "WIDE2",1,
    //"WIDE2", 1,
    "!/%s%sO   /A=%06ld|%s|%s/%s,%d,%i,%i'C, count: %c",
    ax25_base91enc(slat, 4, aprs_lat),
    ax25_base91enc(slng, 4, aprs_lon),
    aprs_alt, stlm, comment,APRS_CALLSIGN, count, errorstatus,temperature1,PiString[17]
      );
    Serial.print(ax25_base91enc(slat, 4, aprs_lat));
    Serial.print(ax25_base91enc(slng, 4, aprs_lon));
    Serial.print("A= ");
    Serial.print(aprs_alt);
    Serial.print(stlm);
