...
#define MTX2_FREQ 144.390 // format 434.XXX  
char callsign[9] = "KE4ERF";  // MAX 9 CHARACTERS!!
...
void prepare_data() {
  ...
  pisignal_v=analogRead(PISIGNAL_ADC)*3.3/1000;
  ...
  pivsmooth[4] = pivsmooth[3];
  pivsmooth[3] = pivsmooth[2];
  pivsmooth[2] = pivsmooth[1];
  pivsmooth[1] = pivsmooth[0];
  pivsmooth[0] = pisignal_v;
  pivaverage = (pivsmooth[0]+pivsmooth[1]+ pivsmooth[2]+pivsmooth[3]+pivsmooth[4])/5;

  Serial.println("Input V");Serial.println(pivaverage);
...
void tx_aprs()
{
  ...
  if(tempsensors==1)
  {
    ...
    "WIDE1", 1, "WIDE2",1,
    //"WIDE2", 1,
    "!/%s%sO   /A=%06ld|%s|%s/%s,%d,%i,%i'C, count: %i",
    ax25_base91enc(slat, 4, aprs_lat),
    ax25_base91enc(slng, 4, aprs_lon),
    aprs_alt, stlm, comment,APRS_CALLSIGN, count, errorstatus,temperature1, pivaverage
      );
